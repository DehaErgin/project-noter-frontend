...
const DataTable = ({
  ...
  const selectedRowsSet = onSelectionChange ? selectedRows : localSelectedRows;
  const setSelectedRowsSet = onSelectionChange || setLocalSelectedRows;

  const handleSelectAll = (checked) => {
    if (checked) {
      const allIds = new Set(data.map(row => row.id));
      setSelectedRowsSet(allIds);
    } else {
  ...
  const isAllSelected = data.length > 0 && data.every(row => selectedRowsSet.has(row.id));
  const isSomeSelected = data.some(row => selectedRowsSet.has(row.id)) && !isAllSelected;
              ...
              {selectable && (
                <th className="px-6 py-3 w-12">
                  <input
                    ...
                    className="w-4 h-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500 dark:border-slate-600 dark:bg-slate-700"
                  />
                </th>
              )}
              {columns.map((column) => (
