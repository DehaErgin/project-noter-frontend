...
const StudentDashboard = () => {
  ...
  if (isLoading) return <LoadingState label="Gathering your dashboard..." />;
  if (error) return <ErrorState message="Dashboard data unavailable." onRetry={refetch} />;
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="p-6 bg-white rounded-2xl shadow-card ring-1 ring-slate-100 dark:bg-slate-900 dark:ring-slate-800">
          ...
          <ul className="mt-4 space-y-2">
            ...
            {!strong.length && <p className="text-sm text-slate-500">No strengths identified yet.</p>}
          </ul>
        </div>
        <div className="p-6 bg-white rounded-2xl shadow-card ring-1 ring-slate-100 dark:bg-slate-900 dark:ring-slate-800">
          ...
          <ul className="mt-4 space-y-2">
            ...
            {!weak.length && <p className="text-sm text-slate-500">No concerns detected.</p>}
          </ul>
        </div>
      </section>
